-- The client_id in academic_qualifications must reference users(id), not clients(id).
-- Client extends User with SINGLE_TABLE inheritance, so client.getId() returns users.id.
-- The clients table has its own separate BIGSERIAL id that is unrelated.
